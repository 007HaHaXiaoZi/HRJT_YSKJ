...
-(void)requetData{
    ...
    NSString *url = @"https://aimap.newayz.com/aimap/ora/v1/scenes?device_type=iosDemo";
...
- (void)PhoneMethodForUnityInitComplete {
    ...
    [[UnityToiOSManger sharedInstance]SetDataRecorderStatus:@"0"];//是否录制hara，1录制 0 不录制
...
NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:@"https://vps.zybervision.com/wayzoom/v1.2/vps/single"]
//NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:@"https://vps.zybervision.com/wayzoom/v1.2/vps/single"]
  cachePolicy:NSURLRequestUseProtocolCachePolicy
  timeoutInterval:10.0];
  ...
  if (error) {
    NSLog(@"error2 %@", error);
      dispatch_async(dispatch_get_main_queue(), ^{
          ...
          NSString *uploadLocationPoint = aa[@"locationInfo"];
          bool cc = [uploadLocationPoint boolValue];
          [YPLogTool YPWLogInfo:rotationti];
          if(cc){
              [self.view makeToast:rotationti];
          }
      });
...
-(void) initARFromAnnotation:(ARSceneBean *)arSceneBean{
    ...
    [YPLogTool YPWLogInfo:@"线上更新时间"];
    [YPLogTool YPWLogInfo:updateTime];
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    NSString *cacheUpdetaTime = [userDefaults stringForKey:arSceneBean.id];
    [YPLogTool YPWLogInfo:@"缓存更新时间"];
    [YPLogTool YPWLogInfo:cacheUpdetaTime];
    ...
    [YPLogTool YPWLogInfo:@"对比更新时间"];
    [[NSString alloc]initWithFormat:@"对比结果：%d",update];
    [YPLogTool YPWLogInfo:[[NSString alloc]initWithFormat:@"对比结果：%d",update]];
    if (local) {
        update = false;
    }
